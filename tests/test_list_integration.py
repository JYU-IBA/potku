...
class TestListIntegration(unittest.TestCase):
    ...
    def test_bins_without_limits(self):
        ...
        # must be defined
        self.assertRaises(ValueError, lambda: li.integrate_bins([0], [1]))
        self.assertEqual(10, li.integrate_bins([0], [1], step_size=10))
        ...
        self.assertEqual(30, li.integrate_bins([0, 1], [1, 2], step_size=10))
        self.assertEqual(0, li.integrate_bins([0, 10], [1, 2], step_size=0))
        self.assertEqual(-7, li.integrate_bins([0, 10, 20], [1, 2, 4], step_size=-1))
    ...
    def test_uneven_axes(self):
        ...
        # axis is exhausted.
        self.assertEqual(2, li.integrate_bins([1, 2], [1, 1, 1]))
        self.assertEqual(1, li.integrate_bins([1, 2, 3], [1]))
    ...
    def test_bad_inputs(self):
        ...
        # range
        self.assertEqual(
            20, li.integrate_bins([0, 1, 2, "foo"], ["foo", 10, 10, 10], a=1, b=1))
    ...
    def test_integrating_with_limits(self):
        ...
        self.assertEqual(
            0, li.integrate_bins(x_axis,
                                 y_axis,
                                 a=3, b=2))

        self.assertEqual(
            20, li.integrate_bins(x_axis,
                                  y_axis,
                                  a=3, b=3))
    ...
    def test_sum_y_values(self):
        ...
        self.assertEqual(sum(y_axis),
                         li.sum_y_values(x_axis, y_axis))

        self.assertEqual(29,
                         li.sum_y_values(x_axis, y_axis, a=4))

        self.assertEqual(21,
                         li.sum_y_values(x_axis, y_axis, b=0))

        self.assertEqual(11 + 12 + 13 + 14,
                         li.sum_y_values(x_axis, y_axis, a=1, b=3))

        self.assertEqual(12,
                         li.sum_y_values(x_axis, y_axis, a=1.5, b=1.5))

        self.assertEqual(0,
                         li.sum_y_values(x_axis, y_axis, a=2, b=1))

    def test_sum_running_avgs(self):
        """Tests sum_running_avgs function"""
        x_axis = [0, 1, 2]
        y_axis = [10, 11, 12]
        self.assertEqual(27, li.sum_running_avgs(x_axis, y_axis))
        self.assertEqual(0, li.sum_running_avgs(x_axis, y_axis, a=2, b=1))
        self.assertEqual(0, li.sum_running_avgs(x_axis, y_axis, a=10))
    ...
    def test_calculate_running_avgs(self):
        ...
        self.assertEqual([(0, 5), (1, 10.5), (2, 11.5)],
                         list(li.calculate_running_avgs(x_axis, y_axis)))

        self.assertEqual([(1, 5.5), (2, 11.5)],
                         list(li.calculate_running_avgs(x_axis, y_axis, a=1)))

        self.assertEqual([(0, 5), (1, 10.5)],
                         list(li.calculate_running_avgs(x_axis, y_axis, b=0)))

        self.assertEqual([(0, 5)],
                         list(li.calculate_running_avgs(x_axis, y_axis, b=-1)))

        self.assertEqual([],
                         list(li.calculate_running_avgs(x_axis, y_axis, a=100)))
    ...
    def test_get_elements_in_range(self):
        ...
        self.assertEqual(list(zip(x_axis, y_axis)),
                         list(li.get_elements_in_range(x_axis, y_axis)))

        # If a > b, nothing is returned
        self.assertEqual([],
                         list(li.get_elements_in_range(x_axis, y_axis, a=2, b=1)))
        ...
        self.assertEqual([(1, 11), (2, 12)],
                         list(li.get_elements_in_range(x_axis, y_axis, a=1, b=1)))

        self.assertEqual([(0, 10)],
                         list(li.get_elements_in_range(x_axis, y_axis, b=-100)))

        # whereas first before a is not
        self.assertEqual([(2, 12)],
                         list(li.get_elements_in_range(x_axis, y_axis, a=1.5, b=1.5)))

        self.assertEqual([],
                         list(li.get_elements_in_range(x_axis, y_axis, a=100)))

        self.assertEqual([(1, 11), (2, 12), (3, 13)],
                         list(li.get_elements_in_range(x_axis, y_axis, a=1, b=2)))

        # Limits can be set for only a or b
        self.assertEqual([(0, 10), (1, 11), (2, 12), (3, 13)],
                         list(li.get_elements_in_range(x_axis, y_axis, b=2)))

        self.assertEqual([(3, 13), (4, 14), (5, 15)],
                         list(li.get_elements_in_range(x_axis, y_axis, a=3)))

        # Function works for any x values that can be compared
        x_axis_str = ["a", "b", "c"]
        y_axis_str = [True, False, None]
        self.assertEqual([("a", True), ("b", False)],
                         list(li.get_elements_in_range(x_axis_str,
                                                       y_axis_str,
                                                       a="a",
                                                       b="a")))
    ...
    def test_incomparable_element_ranges(self):
        ...
        self.assertRaises(TypeError,
                          lambda: list(li.get_elements_in_range(x_axis,
                                                                y_axis)))
