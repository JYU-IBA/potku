...
class AngleCalibrationDialog(QtWidgets.QDialog):
    ...
    def __init__(self, measurements: List[Measurement], detector: Detector,
        ...
        self.AngleSelectorWidget = AngleSelectorWidget(
            self, self.selected_asc_file, self.tof_calibration, self.detector,
     ...
     #       self, self.tof_calibration, old_params)
        ...
        #self.angleSlopeLineEdit.editingFinished.connect(
        ...
        #self.angleSlopeLineEdit.setValidator(double_validator)
        #self.angleOffsetLineEdit.setValidator(double_validator)
    ...
    def set_calibration_parameters_to_parent(self):
        ...
        if self.parent_settings_widget is not None:
            self.parent_settings_widget.angle_slope = float(
                self.slopeLineEdit.text())
            self.parent_settings_widget.angle_offset = float(
    ...
    def set_calibration_parameters_to_parent(self):
        ...
        if self.parent_settings_widget is not None:
            self.parent_settings_widget.angle_slope = float(
                self.angleSlopeLineEdit.text())
            self.parent_settings_widget.angle_offset = float(
                self.angleOffsetLineEdit.text())
            return True
        return False
...
class AngleSelectorWidget(QtWidgets.QWidget):
    ...
    def __init__(self, dialog, asc, tof_calibration, detector: Detector,
                 bin_width: float, column: int, run: Run):
        """Initializes widget.
        ...
        self.matplotlib = MatplotlibAngleSelectorWidget(
            self, detector, tof_calibration, asc, run, bin_width, column,
            dialog)
